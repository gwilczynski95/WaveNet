...
def parse_args(is_training=True):
    if is_training:
        ...
        parser.add_argument('--step', type=int, default=-1, help='A specific step of pre-trained model to use, -1 means'
                                                                 ' the latest')
        parser.add_argument('--seed', type=str, required=True, help='Path to a seed file to generate sound')
